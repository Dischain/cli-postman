...
const insertSendersTable_q = 'insert into senders (email, password) values '	;
...
const insertRecieversTable_q = 'insert into recievers (email) values ';
...
/*runQuery(createSendersTable_q)
	.then(() => { runQuery(insertSendersTable_q, { email: 'sano911@ya.ru', password: 'sanoman11' }); })
	.catch((err) => { console.log(err); })
allQuery(selectAllSenders_q)
	.then((rows) => { console.log(rows); })*/

/*runQuery(createRecieversTable_q)
	.then(() => { runQuery(insertRecieversTable_q, { email: 'pupkin@ya.ru' }); })
	.catch((err) => { console.log(err); })
